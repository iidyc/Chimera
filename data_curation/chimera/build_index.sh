...
Description:
  ...
  By default the builder is executed via micromamba in the chimera env when
  micromamba is available. Set CHIMERA_USE_MICROMAMBA=0 to run directly in the
  current shell environment instead. The old GPU_MVR_USE_MICROMAMBA variable is
...
Outputs:
  dataset/<name>/gpu_search_2m/     when --n-clusters is 2000000 or 2M (default)
  dataset/<name>/gpu_search_1m/     when --n-clusters is 1000000 or 1M
  dataset/<name>/gpu_search_<count>/ for other cluster counts
  backup/log/build/<dataset>/<index_dir>/gpu_build_<run_id>.log
...
index_dir_name_for_n_clusters() {
    ...
    case "$cluster_count" in
        2000000)
            printf '%s' "gpu_search_2m"
            ;;
        1000000)
            printf '%s' "gpu_search_1m"
            ;;
        *)
            printf '%s' "gpu_search_${cluster_count}"
            ;;
    esac
}
...
if [[ ! -x "$gpu_build_bin" ]]; then
    if [[ $dry_run -eq 1 ]]; then
        echo "[dry-run] warning: gpu_build binary not found or not executable: ${gpu_build_bin}" >&2
    else
        die "gpu_build binary not found or not executable: ${gpu_build_bin}"
    fi
fi
