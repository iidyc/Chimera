...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Compare gpu_search_v8 with and without LUT at fixed operating "
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to the LUT-enabled v8 binary. Defaults to <build-dir>/gpu_search_v8.",
    )
    parser.add_argument(
        "--binary-nolut",
        type=Path,
        default=None,
        help=(
            "Path to the no-LUT v8 binary. Defaults to <build-dir>/gpu_search_v8_nolut. "
    ...
    parser.add_argument("--output-dir", type=Path, default=repo_root / "profiling" / "gpu_v8_lut_ablation")
...
def main() -> None:
    ...
    binary_lut = args.binary_lut or (args.build_dir / "gpu_search_v8")
    binary_nolut = args.binary_nolut or (args.build_dir / "gpu_search_v8_nolut")
    ...
    if not binary_nolut.exists():
        raise FileNotFoundError(
            ...
            "The current tree does not build gpu_search_v8_nolut by default."
        )
