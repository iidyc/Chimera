...
def parse_gpu_mvr_log(log_file: Path, repo_root_override: Path | None) -> ParsedOutputs:
    ...
    for raw_line in lines:
        ...
        if match:
            if current_config is not None:
                raise ParseError(
                    f"incomplete GPU-MVR metric block before next config in {log_file}"
                )
        ...
        if match:
            ...
            if missing:
                raise ParseError(
                    f"incomplete GPU-MVR metric block for label={current_config['label']}: missing {', '.join(missing)}"
                )
    ...
    if current_config is not None:
        raise ParseError(f"incomplete trailing GPU-MVR metric block in {log_file}")
    ...
    if expected_count and len(rows) != int(expected_count):
        raise ParseError(
            f"parsed {len(rows)} GPU-MVR rows from {log_file}, expected {expected_count}"
        )
...
def parse_log(log_file: Path, mode: str, repo_root_override: Path | None) -> ParsedOutputs:
    ...
    if effective_mode == "gpu-mvr":
        return parse_gpu_mvr_log(log_file, repo_root_override)
