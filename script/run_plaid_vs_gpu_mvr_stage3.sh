...
usage() {
    cat <<'EOF'
Usage:
  script/run_plaid_vs_gpu_mvr_stage3.sh [options]

Description:
  Prepare Stage-3 focused experiments for GPU-MVR v6 and v8.

Modes:
  chunk-sweep
    Vary overlap_chunks (the current num_chunks control in GPU-MVR) and
    benchmark v6 and v8.
...
Options:
  ...
  --version <v6|v8>        Repeatable. Default: v6 and v8
  --k <top_k>              Retrieval depth. Default: 100
  --label <config_label>   Base GPU-MVR config label. Repeatable.
                           Default: d1 f1 l1
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --version)
            ...
            case "$2" in
                v6|v8) ;;
                *) die "--version must be one of: v6, v8" ;;
            esac
...
case "$mode" in
    ...
    fullbit-efficiency)
        cat <<EOF
[todo] Full-bit efficiency is not exported by the current GPU-MVR source.
...
[todo] Intended future command shape:
       gpu_search_v6/gpu_search_v8 ... --stage3-efficiency-csv <path>
EOF
        exit 0
        ;;
    *)
...
tmp_config="$(mktemp "${TMPDIR:-/tmp}/gpu_mvr_stage3_chunks.XXXXXX.csv")"
cleanup() {
    rm -f "$tmp_config"
}
trap cleanup EXIT
...
for version in "${versions[@]}"; do
    ...
    run_cmd "${cmd[@]}"
    run_cmd python "$parse_logs" --mode gpu-mvr "${profile_log_root}/gpu_search_${version}"
done
