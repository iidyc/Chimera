...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Benchmark gpu_search_v8 with 8 query slots and overlap_chunks "
            "sweep. Uses each dataset config CSV, forces chunks 1/2/4, and "
            "batches all rows per dataset to minimize dataset/index loading."
        )
    )
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=repo_root / "profiling" / "gpu_mvr_v8_slots8_chunk_sweep",
    )
    parser.add_argument("--run-id", default=utc_run_id())
    parser.add_argument("--datasets", nargs="+", choices=DATASETS, default=list(DATASETS))
    parser.add_argument("--config-labels", nargs="+", default=None)
    parser.add_argument("--chunks", nargs="+", type=int, default=[1, 2, 4])
...
def main() -> int:
    ...
    if not args.chunks or any(chunk <= 0 for chunk in args.chunks):
    ...
    config_root = repo_root / "config" / "tmp" / "gpu_mvr_v8_slots8_chunk_sweep" / args.run_id
    rows = []
    chunk_part = "_".join(str(chunk) for chunk in args.chunks)
    for dataset in datasets:
        validate_dataset_inputs(repo_root, dataset)
        source_config = dataset_config_path(repo_root, dataset)
        configs = read_config_rows(source_config)
        if wanted_labels:
        ...
        for config in configs:
            ...
            for chunk in args.chunks:
                for run_index in range(1, args.num_runs + 1):
                    ...
                    run_config["overlap_chunks"] = str(chunk)
                    run_config["label"] = (
                        f"{base_label}_c{chunk}_k{args.k}_s{args.slots}_run{run_index}"
                    )
                    run_configs.append(run_config)
                    config_meta[run_config["label"]] = (base_label, chunk, run_index)

        label_part = "selected" if wanted_labels else "all"
        config_path = (
            config_root / dataset / system.label /
            f"{label_part}_k{args.k}_slots{args.slots}_chunks{chunk_part}_runs{args.num_runs}.csv"
        )
        log_path = (
            log_dir / dataset / system.label /
            f"{label_part}_k{args.k}_slots{args.slots}_chunks{chunk_part}_runs{args.num_runs}.log"
        )
        ...
        print(
            ...
            f"rows={len(run_configs)} k={args.k} slots={args.slots} chunks={chunk_part}",
            flush=True,
        )
        if args.dry_run:
            ...
            with log_path.open("w") as handle:
                ...
                handle.write(f"[driver] source_config_file={source_config}\n")
                handle.write(f"[driver] config_file={config_path}\n")
                handle.write(f"[driver] k={args.k}\n")
                handle.write(f"[driver] slots={args.slots}\n")
                handle.write(f"[driver] chunks={chunk_part}\n")
                handle.flush()
                proc = subprocess.run(
                    cmd,
                    cwd=repo_root,
        ...
        for config in run_configs:
            base_label, chunk, run_index = config_meta[config["label"]]
            row = {
                "experiment": "v8_slots8_chunk_sweep",
                "system": system.label,
                "dataset": dataset,
                "label": base_label,
    ...
    results_csv = output_root / f"v8_slots8_chunk_sweep_results_{args.run_id}.csv"
    summary_csv = output_root / f"v8_slots8_chunk_sweep_summary_{args.run_id}.csv"
    preferred = [
        "experiment", "system", "dataset", "label", "config_label", "chunks",
        "slots", "k", "run_index", "recall_at_10", "recall_at_100", "recall",
        "qps", "avg_latency_ms", "p50_ms", "p90_ms", "p95_ms", "p99_ms",
        "max_ms", "stddev_ms", "gpu_mem_peak_mib",
        "effective_concurrent_queries", "stage3_threads", "nprobe",
    ...
    write_summary(summary_csv, rows, ["system", "dataset", "label", "chunks", "slots", "k"])
