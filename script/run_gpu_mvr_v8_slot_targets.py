...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark gpu_search_v8 with 1,2,4,8,10 slots at Recall@100 target points."
    )
    parser.add_argument("--repo-root", type=Path, default=repo_root)
    parser.add_argument("--build-dir", type=Path, default=repo_root / "gpu-mvr" / "build")
    parser.add_argument("--output-root", type=Path, default=repo_root / "profiling" / "gpu_mvr_v8_slot_targets")
...
def main() -> int:
    ...
    results_csv = output_root / f"v8_slot_targets_results_{args.run_id}.csv"
    summary_csv = output_root / f"v8_slot_targets_summary_{args.run_id}.csv"
    preferred = [
