...
inline double percentile_ms(const std::vector<double>& sorted_ms, double pct) {
    if (sorted_ms.empty()) {
        return 0.0;
    }
...
inline QueryLatencySummary summarize_query_latencies_ms(const std::vector<double>& latencies_ms) {
...
inline void print_query_latency_summary(
