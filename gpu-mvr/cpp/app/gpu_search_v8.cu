...
void apply_runtime_config(gpu_mvr_index& index, const gpu_search_runtime_options& runtime) {
...
struct V8ProfileArgs {
    std::string latency_csv = "v8_profile_latency.csv";
};
...
std::vector<std::string> strip_v8_profile_args(
    int argc,
    char** argv,
    V8ProfileArgs& profile_args) {
...
int main(int argc, char** argv) {
    ...
    try {
        forwarded_args = strip_v8_profile_args(argc, argv, profile_args);
    ...
    std::cout
        << "[RUN] Constructing shared v8 index with " << query_slots
    ...
    auto owner_index = std::make_unique<gpu_mvr_index>(
        args.index_file,
        doclens,
        allocation_runtime);
    std::vector<std::unique_ptr<gpu_mvr_index>> extra_slots;
    extra_slots.reserve(std::max(0, query_slots - 1));
    std::vector<gpu_mvr_index*> indices;
    indices.reserve(query_slots);
    indices.push_back(owner_index.get());
    for (int slot = 1; slot < query_slots; ++slot) {
        extra_slots.push_back(std::make_unique<gpu_mvr_index>(
            *owner_index,
            allocation_runtime));
    ...
    for (const auto& runtime_config : runtime_configs) {
        ...
        if (args.profile_eval_all_queries) {
            ...
            for (int slot = 0; slot < query_slots; ++slot) {
                workers.emplace_back([&, slot]() {
                    ...
                    omp_set_num_threads(effective_stage3_threads);
                    gpu_mvr_index* index = indices[slot];
                    while (true) {
        ...
        report_v8_recalls(eval_ground_truth, results, args.k);
#ifdef GPU_MVR_V8_PROFILE_APP
