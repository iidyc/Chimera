    ...
    size_t cluster_size)
{
    ...
    std::vector<uint8_t> source_codes(batch_size * stage1_cache.code_bytes_per_vector());
...
#pragma omp parallel for
    for (size_t idx = 0; idx < input_ids.size(); ++idx) {
        ...
        for (int j = 0; j < static_cast<int>(q_doclen); ++j) {
            const auto& lut = *luts[static_cast<size_t>(j)];
            ...
            for (size_t batch_idx = batch_begin; batch_idx <= batch_end; ++batch_idx) {
                ...
                for (size_t tid = overlap_begin; tid < overlap_end; ++tid) {
                    ...
                    const float dist =
                        (ip - queries[j].cb1_sumq) * stage2_cache.factors()[batch_token_start + lane];
                    token_dists[(candidate_doc_ptrs[idx] + local_token_idx) * q_doclen + j] = dist;
