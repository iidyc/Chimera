...
void v7_bind_compact_doc_arena(
    gpu_mvr_index_v7_base::Workspace& ws,
    char* base,
    size_t row_capacity,
...
gpu_mvr_index::gpu_mvr_index(
    ...
    : gpu_mvr_index_v7_base(filename, doc_lens, runtime_options) {
    const char* stage1_mode_env = std::getenv("GPU_MVR_V7_STAGE1_MODE");
    if (stage1_mode_env != nullptr) {
        ...
        if (mode == "memory_padded_align" || mode == "padded") {
            ...
            throw std::runtime_error(
                "Unknown GPU_MVR_V7_STAGE1_MODE=" + mode +
...
void gpu_mvr_index::maybe_load_stage1_padded_layout(const std::string& filename) {
...
void gpu_mvr_index::allocate_workspace() {
    ...
    for (int i = 0; i < gpu_mvr_index_v7_base::Workspace::PST_NUM_D2H_CHUNKS; ++i) {
    ...
    for (int i = 0; i < gpu_mvr_index_v7_base::Workspace::MAX_XFER_RECORDS; ++i) {
...
#ifdef GPU_MVR_OVERLAP_STAGE23
    gpu_mvr_index_v7_base::rank_stage23_persistent_impl<kProfile>(
        actual_k_stage1,
        k,
        k_rank_all_tokens,
        query_objs.data(),
        result);
#else
#error "gpu_search_v7 requires GPU_MVR_OVERLAP_STAGE23"
#endif
