...
inline float distance_one_bit(query_object* query, const char* binary_code, float one_bit_factor, size_t padded_dim) {
...
inline float ip_ex_bits(query_object* query, const char* ex_code, float (*ip_func_)(const float*, const uint8_t*, size_t), size_t padded_dim) {
    return ip_func_(query->rotated_query, (uint8_t*)ex_code, padded_dim);
}

inline float combine_dists(query_object* query, float one_bit_dist, float ex_bits_dist, float one_bit_factor, float ex_factor, size_t ex_bits) {
