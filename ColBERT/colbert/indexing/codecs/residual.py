...
class ResidualCodec:
    ...
    def __init__(self, config, centroids, avg_residual=None, bucket_cutoffs=None, bucket_weights=None):
        ...
        if self.use_gpu > 0:
            ...
            self.centroids_cpu = centroids.float()
        else:
        ...
        if torch.is_tensor(bucket_cutoffs):
            if self.use_gpu:
                ...
                self.bucket_weights_cpu = bucket_weights.float()
        ...
        if self.use_gpu:
            self.reversed_bit_map_cpu = self.reversed_bit_map
            self.reversed_bit_map = self.reversed_bit_map.cuda()
            if self.decompression_lookup_table is not None:
                self.decompression_lookup_table_cpu = self.decompression_lookup_table
