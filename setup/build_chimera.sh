...
usage() {
    cat <<'EOF'
Usage: setup/build_chimera.sh [--cpu-only] [--target <name>] [--build-dir <path>] [-- <extra-cmake-configure-args...>]

Options:
  --cpu-only          Configure with -DCHIMERA_ENABLE_GPU=OFF and use build-cpu-only by default.
...
Examples:
  ...
  setup/build_chimera.sh --cpu-only
  setup/build_chimera.sh --cpu-only --target cpu_search
  setup/build_chimera.sh -- -DCMAKE_CXX_COMPILER=clang++
EOF
}
...
if [[ "${cpu_only}" -eq 1 ]]; then
    cmake_configure_args+=(-DCHIMERA_ENABLE_GPU=OFF)
fi
