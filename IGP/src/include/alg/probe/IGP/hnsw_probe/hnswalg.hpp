...
class HierarchicalNSW : public AlgorithmInterface<dist_t> {
 public:
  ...
  uint32_t ef_ = 8; // nprobe + addition_search_size_
  ...
  void inline set_query_info(const uint32_t query_n_vec, const uint32_t vec_dim,
    ...
    this->nprobe_ = nprobe;
    this->ef_ = nprobe + addition_search_size_;

    this->qvec_heap_l_.resize(query_n_vec);
    for (uint32_t qvecID = 0; qvecID < query_n_vec; qvecID++) {
      qvec_heap_l_[qvecID] = SearchResultMinHeap(ef_, maxelements_);
    }
  ...
  void search_topk(const uint32_t qvecID, uint32_t &n_computation) {
    ...
    const std::vector<SearchResultEle> ele_l = qvec_heap_l_[qvecID].PopTopScore(nprobe_);
    const uint32_t n_cand = ele_l.size();
    assert(ele_l.size() <= nprobe_);
    for (uint32_t candID = 0; candID < n_cand; candID++) {
      ...
      sorted_probe_ele_l_[score_offset + candID] = ProbeEle(qvecID, centID, score);
    }
