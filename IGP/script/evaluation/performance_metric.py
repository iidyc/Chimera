...
def read_mrr_groundtruth_jsonl(dataset: str, username: str):
    del username
    gnd_jsonl_filename = dataset_io.rawdata_dir(dataset) / 'document' / 'queries.gnd.jsonl'
    end2end_gnd_m = {}
...
def read_mrr_passageID_l(dataset: str, username: str):
    del username
    collection_filename = dataset_io.rawdata_dir(dataset) / 'document' / 'collection.tsv'
    end2end_passageID_l = []
...
def read_end2end_success_gnd(dataset: str, username: str):
    del username
    gnd_jsonl_filename = dataset_io.rawdata_dir(dataset) / 'document' / 'queries_short_answer.gnd.jsonl'
    end2end_gnd_m = {}
    with open(gnd_jsonl_filename, 'r', encoding="utf-8") as f:
        for line in f:
            query_gnd_json = json.loads(line)
...
def load_groundtruth(username: str, dataset: str, topk: int):
    embedding_dir = str(dataset_io.embedding_dir(dataset))
    rawdata_path = str(dataset_io.rawdata_dir(dataset))
...
def count_accuracy(username: str, dataset: str, topk: int,
    ...
    del username
    answer_dir = str(dataset_io.result_answer_dir())
...
def count_accuracy_by_baseline_ip(username: str, dataset: str, topk_l: list,
    ...
    embedding_dir = str(dataset_io.embedding_dir(dataset))
    answer_dir = str(dataset_io.result_answer_dir())
    performance_dir = str(dataset_io.result_performance_dir())
...
def count_accuracy_by_ID(username: str, dataset: str, topk: int,
    ...
    queryID_l = dataset_io.load_query_ids(
        username=username,
        dataset=dataset,
        num_queries=len(baseline_id_l),
    )
...
def count_accuracy_by_ID_ip(username: str, dataset: str, topk: int,
    ...
    query_l = dataset_io.load_query_embeddings(username="", dataset=dataset)
