...
def gnd_cpp(username: str, dataset: str, topk_l: list, n_batch_read: int = 1,
    ...
    accu_itemID = 0
    pending_chunks = []
    for itemlen_l, item_vecs_l in dataset_io.iter_embedding_chunks(username=username, dataset=dataset):
        pending_chunks.append((itemlen_l, item_vecs_l))
        if len(pending_chunks) < n_batch_read:
            continue

        start_load_time = time.time()
        itemlen_batch_l = np.concatenate([chunk[0] for chunk in pending_chunks]).astype(np.uint32)
        item_vecs_batch_l = np.concatenate([chunk[1] for chunk in pending_chunks], axis=0).reshape(-1, vec_dim)
        n_item_batch_chunk = len(itemlen_batch_l)

        item_l = [
            util.item_vecs_in_chunk(
                vecs_l=item_vecs_batch_l,
                itemlen_l=itemlen_batch_l,
                itemID=itemID_batch_chunk) for itemID_batch_chunk in range(n_item_batch_chunk)
        ]
    ...
    if pending_chunks:
        ...
        for topk_i, topk in enumerate(topk_l, 0):
            distance_l, id_l = index.searchKNN(topk=topk)
            tmp_final_distance_l, tmp_final_id_l = module.DocRetrieval.merge_result(
                np.array(final_distance_l[topk_i]),
                np.array(final_id_l[topk_i]),
                distance_l,
                id_l)
            final_distance_l[topk_i] = tmp_final_distance_l
...
def save_gnd_npy(gnd_dist_l: np.ndarray, gnd_id_l: np.ndarray, username: str, dataset: str, topk: int):
    ...
    embedding_dir = dataset_io.embedding_dir(dataset)
...
def save_gnd_tsv(gnd_dist_l: np.ndarray, gnd_id_l: np.ndarray, username: str, dataset: str, topk: int):
    ...
    groundtruth_filename = os.path.join(
        embedding_dir,
        f'{dataset}-groundtruth-top{topk}-{build_index_suffix}-{retrieval_suffix}.tsv')

    qID_l = dataset_io.load_query_ids(username=username, dataset=dataset, num_queries=len(gnd_dist_l))
