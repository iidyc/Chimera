...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Parse cpu_search aggregate profiling logs into a CSV summary."
        )
    )
    parser.add_argument(
        "--batch-id",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output CSV path. Defaults to profiling/cpu_search/profile_<batch>.csv.",
    )
    parser.add_argument(
        "paths",
        nargs="+",
...
def main() -> int:
    ...
    try:
        ...
        if output_csv is None:
            output_csv = Path("profiling") / "cpu_search" / f"profile_{args.batch_id}.csv"
