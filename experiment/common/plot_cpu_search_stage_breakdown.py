...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Create stacked bar plots from cpu_search profile summary CSV output."
        )
    )
    parser.add_argument(
        "--profile-csv",
        type=Path,
        required=True,
        help="CSV produced by parse_cpu_search_profile_logs.py.",
    )
    parser.add_argument(
        "--output-prefix",
        type=Path,
...
def main() -> int:
    ...
    ax.set_title("cpu_search average end-to-end breakdown")
    ax.set_xticks(x)
