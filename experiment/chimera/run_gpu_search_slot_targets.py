...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark gpu_search with 1,2,4,8,10 slots at Recall@100 target points."
    )
    parser.add_argument("--repo-root", type=Path, default=repo_root)
    parser.add_argument("--build-dir", type=Path, default=repo_root / "Chimera" / "build")
    parser.add_argument("--output-root", type=Path, default=repo_root / "profiling" / "chimera_slot_targets")
    parser.add_argument("--run-id", default=utc_run_id())
    parser.add_argument("--datasets", nargs="+", choices=DATASETS, default=list(DATASETS))
    parser.add_argument("--targets", nargs="+", default=["0.90", "0.95"])
    parser.add_argument("--slots", nargs="+", type=int, default=[1, 2, 4, 8, 10])
...
def main() -> int:
    ...
    if any(slot <= 0 for slot in args.slots):
        raise ValueError("--slots must all be > 0")
    if args.build:
        ...
        for cfg in configs:
            ...
            for slots in args.slots:
                log_path = (
                    ...
                    f"slots{slots}_run{run_index}.log"
                )
                rows.append(run_gpu_search_direct(
                    ...
                    args.dry_run,
                    env,
                    concurrent_queries=slots,
                    stage3_threads=args.stage3_threads,
                    extra_row={
                        "experiment": "chimera_slot_targets",
    ...
    if args.dry_run:
        return 0
    results_csv = output_root / f"chimera_slot_targets_results_{args.run_id}.csv"
    summary_csv = output_root / f"chimera_slot_targets_summary_{args.run_id}.csv"
    preferred = [
        "experiment", "system", "dataset", "target_recall", "base_label",
        "slots", "k", "run_index", "recall_at_10", "recall_at_100",
        "recall", "qps", "avg_latency_ms", "p50_ms", "p90_ms", "p95_ms",
        "p99_ms", "max_ms", "stddev_ms", "gpu_mem_peak_mib",
