...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Run gpu_search at Recall@100 ~= 0.95 while sweeping concurrent query slots. "
    ...
    parser.add_argument("--output-root", type=Path, default=repo_root / "profiling" / "chimera_slot_sweep")
    ...
    parser.add_argument(
        ...
        default=["gpu_search", "gpu_search_nolut"],
    )
...
def main() -> int:
    ...
    if args.stage3_threads > 0 and args.stage3_threads == args.omp_num_threads:
        print(
            "[driver] stage3_threads equals omp_num_threads; using gpu_search auto-split "
    ...
    if args.dry_run:
        return 0
    results_csv = output_root / f"chimera_slot_sweep_results_{args.run_id}.csv"
    summary_csv = output_root / f"chimera_slot_sweep_summary_{args.run_id}.csv"
    preferred = [
        "system",
        "dataset",
        "target_recall",
