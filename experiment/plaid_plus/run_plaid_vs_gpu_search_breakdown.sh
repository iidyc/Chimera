...
usage() {
    cat <<'EOF'
Usage:
  experiment/plaid_plus/run_plaid_vs_gpu_search_breakdown.sh [options]
...
Description:
  ...
  Current support:
    ...
    - legacy gpu_search/chimera: source support is still missing for aggregated per-run
...
for k in "${k_values[@]}"; do
    for dataset in "${datasets[@]}"; do
        ...
        cat <<EOF
[todo] gpu_search aggregate breakdown is not implemented yet.
[todo] intended future output:
       ${output_root}/breakdown/k${k}/${dataset}/gpu_search/profile_breakdown.csv
[todo] intended future command shape:
       gpu_search --config-file profiling/gpu_search_config.csv --k ${k} --profile-breakdown-csv <path>
EOF
    done
done
