...
usage() {
    cat <<'EOF'
Usage:
  experiment/plaid_plus/run_plaid_vs_gpu_search_timeline.sh [options]
...
Options:
  ...
  --gpu-label <config_label> gpu_search config label. Default: l1
...
resolve_gpu_index_dir() {
    ...
    for candidate in \
        ...
        "${dataset_dir}/${legacy_prefix}_index"; do
...
echo "[note] gpu_search timeline readability will improve when NVTX ranges are enabled."
...
run_cmd nsys profile --force-overwrite=true --trace=cuda,nvtx,osrt \
    --output "${timeline_root}/gpu_search_${gpu_label}_k${k}" \
    "${build_dir}/gpu_search" \
    --query "$query_file" \
