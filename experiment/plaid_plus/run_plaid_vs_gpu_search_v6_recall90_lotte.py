...
def parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(
        description=(
            ...
            "PLAID (ColBERT) and gpu_search_v6/gpu_search using fixed configs."
        )
    )
    p.add_argument("--repo-root", type=Path, default=repo_root)
    p.add_argument("--output-dir", type=Path, default=default_output)
    p.add_argument("--build-dir", type=Path, default=repo_root / "Chimera" / "build")
    ...
    p.add_argument(
        "--systems",
        nargs="+",
        choices=["plaid", "gpu_search_v6", "gpu_search"],
...
def dataset_paths(args: argparse.Namespace, dataset: str) -> dict[str, Path]:
    ...
    gpu_index = next(
        (path for path in index_candidates if path.is_dir() and (path / "doclens.bin").is_file()),
        index_candidates[0],
    )
    return {
        "root": dataset_root,
...
def run_gpu_profile(args: argparse.Namespace, dataset: str, system: str, output_dir: Path) -> tuple[Path, dict[str, float]]:
    binary = args.build_dir / system
    use_avg_profile = system in {"gpu_search_v6", "gpu_search"}
    paths = dataset_paths(args, dataset)
    log_path = output_dir / (
...
def main() -> int:
    ...
    for dataset in args.datasets:
        ...
        for system in ("gpu_search_v6", "gpu_search"):
        ...
        gpu_recalls = {
            ...
            if summary.system in {"gpu_search_v6", "gpu_search"}
        }
        if len(gpu_recalls) == 2:
            v6_recall = gpu_recalls["gpu_search_v6"]
            chimera_recall = gpu_recalls["gpu_search"]
            if abs(v6_recall - chimera_recall) > 1e-9:
                raise RuntimeError(
                    "gpu_search_v6 and gpu_search should have identical recall in this script, "
                    f"but got {v6_recall:.12f} vs {chimera_recall:.12f}"
                )
