...
#if CHIMERA_STAGE1_SPARSE_CLEANUP == 1
...
std::vector<size_t> chimera_index::search_impl(const float* queries, size_t k) {
        CHIMERA_NVTX_SCOPE("gpu_index.search", 0xFF2A9D8F);
#ifdef CHIMERA_PROFILE
        ...
        {
            CHIMERA_NVTX_SCOPE("search.rotate_queries_cpu", 0xFF457B9D);
        ...
        {
            CHIMERA_NVTX_SCOPE("search.build_query_objects", 0xFF577590);
            for (size_t i = 0; i < Q_DOCLEN; ++i) {
        ...
        {
            CHIMERA_NVTX_SCOPE("search.h2d_queries", 0xFF4D908E);
            if constexpr (kProfile) {
        ...
        {
            CHIMERA_NVTX_SCOPE("stage0.precompute_lut", 0xFF43AA8B);
            Chimera::precompute_binary_ip_state(
                ws_.d_queries,
#if CHIMERA_BINARY_IP_IMPL == 2
                ws_.d_lut,
#else
...
void chimera_index::rank_cluster_dists_gpu_impl(
        ...
        CHIMERA_NVTX_SCOPE("stage1.rank_cluster_dists_gpu", 0xFFF4A261);
        int total_pairs = 0;
        int h_num_touched = 0;
...
#if CHIMERA_STAGE1_SPARSE_CLEANUP == 0
        ...
        {
            CHIMERA_NVTX_SCOPE("stage1.cagra_search", 0xFFE76F51);
        ...
        {
            CHIMERA_NVTX_SCOPE("stage1.expand_candidates", 0xFFE9C46A);
...
#endif
            {
                CHIMERA_NVTX_SCOPE("stage1.binary_ip", 0xFFE76F51);
                Chimera::launch_stage1_binary_ip(
                    ws_.d_queries,
#if CHIMERA_BINARY_IP_IMPL == 2
                    ws_.d_lut,
#else
...
#endif
            {
                CHIMERA_NVTX_SCOPE("stage1.aggregate_scores", 0xFFF4A261);
...
void chimera_index::rank_all_tokens_1bit_gpu(
...
template <bool kProfile>
void chimera_index::rank_all_tokens_1bit_gpu_impl(
        int num_candidates,  // from Stage 1
        size_t k,
        std::vector<size_t>& output_ids,
        std::vector<float>& one_bit_dists,
        ...
        CHIMERA_NVTX_SCOPE("stage2.rank_all_tokens_1bit_gpu", 0xFF264653);
        ...
        Chimera::launch_stage2_binary_ip(
            ws_.d_queries,
#if CHIMERA_BINARY_IP_IMPL == 2
            ws_.d_lut,
#else
...
void chimera_index::rank_stage23_persistent_impl(
        ...
        CHIMERA_NVTX_SCOPE("stage23.rank_stage23_persistent", 0xFF2A9D8F);
        if (num_candidates == 0) return;
...
#pragma omp parallel for schedule(static)
        ...
        for (int c = 0; c < actual_chunks; c++) {
            ...
            std::vector<Chimera::RefineTask> refine_tasks;
            refine_tasks.reserve(to_refine);
            for (int i = 0; i < to_refine; ++i) {
                const int doc_id = h_candidate_doc_ids[h_sel_indices[i]];
                refine_tasks.push_back(Chimera::RefineTask{
                    i,
                    doc_id,
                    h_out_offsets[i],
                    (size_t)(doc_ptrs_[doc_id + 1] - doc_ptrs_[doc_id])
...
#if CHIMERA_CPU_REFINE_IMPL == 1
            Chimera::v1::compute_ip_ex_bits_cpu(*this, queries, refine_tasks, ip_ex_buf.data());
#else
            Chimera::v2::compute_ip_ex_bits_cpu(
...
void chimera_index::rank_all_tokens_exbits_cpu(
        query_object* queries,
...
#pragma omp parallel for schedule(dynamic)
...
#else
        {
            ...
            const auto assignment = Chimera::v2::build_token_balanced_buckets(refine_tasks, thread_count);
#pragma omp parallel num_threads(thread_count)
            {
                const int tid = omp_get_thread_num();
