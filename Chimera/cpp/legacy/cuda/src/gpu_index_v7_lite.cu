#include "gpu_index_v7_lite.cuh"

#define CHIMERA_V3_SKIP_WORKSPACE_ALLOC 1
#define chimera_index chimera_index_v7_lite_base
...
void v7_bind_compact_doc_arena(
    chimera_index_v7_lite_base::Workspace& ws,
    char* base,
    size_t row_capacity,
...
chimera_index::chimera_index(
    ...
    : chimera_index_v7_lite_base(filename, doc_lens, runtime_options) {
    std::cout << "[" << CHIMERA_IMPL_NAME << "] Stage1 align mode: compact_align";
#if defined(CHIMERA_V7_DISABLE_STAGE1_SHIFT) && CHIMERA_V7_DISABLE_STAGE1_SHIFT
    std::cout << " without shifted cluster loads\n";
#else
    std::cout << " with shifted cluster loads\n";
#endif
...
void chimera_index::allocate_workspace() {
...

    {
        ...
        auto xform_iter = thrust::make_transform_iterator(len_ptr, cast_int_size_t_v7_lite_base());
        temp_bytes = 0;
    ...
    for (int i = 0; i < chimera_index_v7_lite_base::Workspace::PST_NUM_D2H_CHUNKS; ++i) {
    ...
    for (int i = 0; i < chimera_index_v7_lite_base::Workspace::MAX_XFER_RECORDS; ++i) {
...
std::vector<size_t> chimera_index::search_impl(const float* queries, size_t k) {
    ...
    chimera_index_v7_lite_base::rank_stage23_persistent_impl<kProfile>(
        actual_k_stage1,
        k,
        k_rank_all_tokens,
        query_objs.data(),
...
void chimera_index::rank_cluster_dists_gpu_impl(
    ...
    stage1_binary_ip_lut_flag_docs_v7_lite_kernel<<<grid, threads_per_block, 0, stream>>>(
        ws_.d_cagra_labels,
        d_cluster_pos_,
    ...
    bitmap_offset_init_v7_lite_kernel<<<
    ...
    bitmap_unique_docs_v7_lite_kernel<<<
        (doc_bitmap_bucket_count_ + threads_per_block - 1) / threads_per_block,
        threads_per_block,
        0,
        stream>>>(
...
#endif
    stage1_binary_ip_lut_v7_lite_kernel<<<grid, threads_per_block, 0, stream>>>(
        ws_.d_lut,
        d_clustered_code_,
        d_clustered_factor_,
        ws_.d_cb1_sumq,
...
#endif
    sum_doc_scores_compact_v7_lite_kernel<<<sparse_blocks, thread_count, 0, stream>>>(
        ws_.d_doc_query_max,
        ws_.d_unique_doc_ids,
        ws_.d_stage1_doc_scores,
        ws_.d_topk_doc_ids,
