...
__global__ void bitmap_offset_init_v7_kernel(
    const doc_bitmap_bucket_t* __restrict__ d_doc_bitmap,
    size_t num_buckets,
...
__global__ void bitmap_unique_docs_v7_kernel(
    const doc_bitmap_bucket_t* __restrict__ d_doc_bitmap,
    size_t num_buckets,
    const doc_bitmap_offset_t* __restrict__ d_doc_bitmap_offsets,
...
__global__ void sum_doc_scores_compact_v7_kernel(
...
__global__ void stage1_binary_ip_lut_flag_docs_v7_kernel(
...
__global__ void stage1_binary_ip_lut_flag_docs_v7_padded_kernel(
    const uint32_t* __restrict__ d_cagra_labels,
    const uint2*    __restrict__ d_cluster_meta,
    const int*      __restrict__ d_clustered_doc_ids,
    doc_bitmap_bucket_t* __restrict__ d_doc_bitmap,
    size_t          num_docs,
    int             nprobe,
    ...
    for (int p = threadIdx.x; p < nprobe; p += blockDim.x) {
        const uint32_t label = my_labels[p];
        if (label < static_cast<uint32_t>(n_clusters)) {
            const uint2 meta = d_cluster_meta[label];
            smem_cstart[p] = meta.x;
            smem_prefix[p + 1] = static_cast<int>(meta.y);
        } else {
            smem_cstart[p] = 0;
            smem_prefix[p + 1] = 0;
        }
    ...
    for (int flat_idx = threadIdx.x + blockIdx.x * blockDim.x;
        ...
        const int cluster_local = flat_idx - smem_prefix[lo];
        const uint32_t emb_pos = smem_cstart[lo] + static_cast<uint32_t>(cluster_local);
        const int doc_id = d_clustered_doc_ids[emb_pos];
...
__global__ void stage1_binary_ip_lut_v7_kernel(
...
__global__ void stage1_binary_ip_lut_v7_padded_kernel(
    const float*    __restrict__ d_lut,
    const char*     __restrict__ d_clustered_code,
    const float*    __restrict__ d_clustered_factor,
    const float*    __restrict__ d_cb1_sumq,
