...
__global__ void doc_score_all_tokens_kernel(
    const float*  __restrict__ d_token_dists,
    const size_t* __restrict__ d_candidate_offsets,
    float*        d_doc_scores,
    size_t total_tokens,
    ...
    size_t start = d_candidate_offsets[cand_idx];
    size_t end = (cand_idx + 1 < num_candidates)
        ? d_candidate_offsets[cand_idx + 1]
        : total_tokens;

    float score = 0.0f;
    for (size_t i = start; i < end; i++) {
        for (int q = 0; q < Q_DOCLEN; q++) {
            score += d_token_dists[i * Q_DOCLEN + q];
        }
    }
