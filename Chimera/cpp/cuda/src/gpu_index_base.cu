...
chimera_index_base::chimera_index_base(
...
#ifndef CHIMERA_BASE_SKIP_WORKSPACE_ALLOC
    allocate_workspace();
...
void chimera_index_base::set_doc_mapping(const std::vector<int>& doc_lens) {
...
void chimera_index_base::compute_workspace_probe_bounds() {
...
void chimera_index_base::allocate_workspace() {
...
size_t chimera_index_base::doc_len(size_t doc_id) const {
    return doc_ptrs_[doc_id + 1] - doc_ptrs_[doc_id];
}

// ======================== SEARCH PIPELINE ========================

std::vector<size_t> chimera_index_base::search(const float* queries, size_t k) {
    return search_impl<false>(queries, k);
}

std::vector<size_t> chimera_index_base::search_profiled(const float* queries, size_t k) {
    return search_impl<true>(queries, k);
}

template <bool kProfile>
std::vector<size_t> chimera_index_base::search_impl(const float* queries, size_t k) {
...
void chimera_index_base::rank_cluster_dists_gpu(
...
template <bool kProfile>
void chimera_index_base::rank_cluster_dists_gpu_impl(
    query_object* h_query_objs,
    size_t nprobe, size_t k,
    int& actual_k_out,
    cudaStream_t stream
...
void chimera_index_base::rank_stage23_persistent(
    int num_candidates,
    size_t k,
    size_t k_stage2,
...
template <bool kProfile>
void chimera_index_base::rank_stage23_persistent_impl(
    int num_candidates,
    size_t k,
    size_t k_stage2,
    query_object* queries,
...
#endif
#ifdef CHIMERA_STAGE2_USE_V0_DOC_SCORE
        int score_blocks = (c_count + score_threads - 1) / score_threads;
        doc_score_all_tokens_kernel<<<score_blocks, score_threads, 0, stream>>>(
            ws_.d_token_dists,
...
void chimera_index_base::cpu_refine_scores(
