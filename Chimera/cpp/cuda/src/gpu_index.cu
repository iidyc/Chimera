...
#define CHIMERA_BASE_SKIP_WORKSPACE_ALLOC 1
#define chimera_index chimera_index_base
#define cast_int_size_t cast_int_size_t_base
#include "gpu_index_base.cu"
#undef cast_int_size_t
#undef chimera_index
#undef CHIMERA_BASE_SKIP_WORKSPACE_ALLOC
...
size_t compact_sum_top_sorted_counts(std::vector<size_t>& counts, size_t top_n) {
...
std::pair<size_t, size_t> compact_compute_stage2_token_bounds(
...
size_t compute_initial_compact_doc_capacity(
    size_t num_tokens,
    size_t num_clusters,
    size_t max_nprobe,
...
size_t grow_compact_doc_capacity(size_t current, size_t required, size_t limit) {
    ...
    const size_t required_slack =
        std::max(required / 16, kCompactDocMinSlackRows);
    const size_t current_slack =
        std::max(current / 8, kCompactDocMinSlackRows);
...
void bind_compact_doc_arena(
    chimera_index_base::Workspace& ws,
    char* base,
    size_t row_capacity,
...
chimera_index::chimera_index(
    ...
    : chimera_index_base(filename, doc_lens, runtime_options) {
    std::cout << "[gpu_search] Stage1 align mode: compact_align with shifted cluster loads\n";
...
#ifdef CHIMERA_STAGE2_DOC_LAYOUT
...
#pragma omp parallel for schedule(dynamic, 256)
    ...
    workspace_probe_token_bound_ =
        compact_sum_top_sorted_counts(cluster_token_counts, probe_cluster_bound);
    workspace_probe_unique_doc_bound_ =
        compact_sum_top_sorted_counts(cluster_unique_doc_counts, probe_cluster_bound);
}
...
void chimera_index::ensure_compact_doc_capacity(size_t required_rows) {
    ...
    const size_t grown_rows =
        (compact_doc_capacity_ == 0)
            ? std::min(
                  ...
                  align_up(required_rows, kCompactDocRowAlignment))
            : grow_compact_doc_capacity(
    ...
    std::cout << "[workspace][gpu_search] Growing compact doc capacity from "
    ...
    CUDA_CHECK(cudaMalloc(
        ...
        compact_doc_buffer_bytes_ + kCompactDocBufferAlignment - 1));
    d_compact_doc_buffer_ =
        align_up_ptr(d_compact_doc_buffer_raw_, kCompactDocBufferAlignment);
    bind_compact_doc_arena(ws_, d_compact_doc_buffer_, grown_rows, required_topk_capacity);
...
void chimera_index::allocate_workspace() {
    ...
    std::tie(ws_.max_stage2_tokens, ws_.max_stage2_k_tokens) =
        compact_compute_stage2_token_bounds(doc_ptrs_, ws_.max_stage2_candidates, ws_.max_stage2_k);
    ...
    const size_t initial_compact_doc_capacity = compute_initial_compact_doc_capacity(
        n,
        n_clusters,
    ...
    std::cout << "Initial GPU memory required for gpu_search workspace: "
...

    {
        ...
        auto xform_iter = thrust::make_transform_iterator(len_ptr, cast_int_size_t_base());
        temp_bytes = 0;
    ...
    for (int i = 0; i < chimera_index_base::Workspace::PST_NUM_D2H_CHUNKS; ++i) {
    ...
    for (int i = 0; i < chimera_index_base::Workspace::MAX_XFER_RECORDS; ++i) {
...
std::vector<size_t> chimera_index::search_impl(const float* queries, size_t k) {
    ...
    chimera_index_base::rank_stage23_persistent_impl<kProfile>(
        actual_k_stage1,
        k,
        k_rank_all_tokens,
        query_objs.data(),
...
void chimera_index::rank_cluster_dists_gpu_impl(
    ...
    if (static_cast<size_t>(h_num_touched) > max_compact_docs_) {
        throw std::runtime_error(
            "gpu_search compact-doc count " + std::to_string(h_num_touched) +
...
#endif
#ifdef CHIMERA_USE_LUT
    stage1_binary_ip_lut_compact_kernel<<<grid, threads_per_block, 0, stream>>>(
        ws_.d_lut,
        d_clustered_code_,
        d_clustered_factor_,
        ws_.d_cb1_sumq,
        ...
        ivf->n_clusters);
#else
    stage1_binary_ip_nolut_compact_kernel<<<grid, threads_per_block, 0, stream>>>(
        ws_.d_queries,
        d_clustered_code_,
        d_clustered_factor_,
        ws_.d_cb1_sumq,
...
#endif
    sum_doc_scores_compact_kernel<<<sparse_blocks, thread_count, 0, stream>>>(
        ws_.d_doc_query_max,
        ws_.d_unique_doc_ids,
        ws_.d_stage1_doc_scores,
        ws_.d_topk_doc_ids,
